...
const submitOrder = async (type) => {
    ...
    try {
        const response = await axios.post('https://localhost:44302/PurchasingMng' + url, form);
...
const submit = async () => {
    ...
    try {
        const response = await axios.post('https://localhost:44302/PurchasingMng/PurchaseConfirmed', form);
        const data = response.data;
        if (data.state === 'success') {
