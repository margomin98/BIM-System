...
const getAlertMsg = async () => {
  try {
    const response = await axios.post('http://192.168.0.117:7008/HomePage/GetAlertMessage', '');
...
const changeIsExcute = async (itemData) => {
  ...
  try {
    const response = await axios.post('http://192.168.0.117:7008/HomePage/SetExecuteOfDeliveredItem', { OM_ID: itemData.OM_ID, IsExecute: itemData.IsExecute });
    const data = response.data;
...
const getLineNotificationStatus = async () => {
  try {
    const response = await axios.post('http://192.168.0.117:7008/HomePage/GetLineSetting', '');
    const data = response.data;
    if (data.state === 'success') {
      // 有開啟
...
const handleLineSwitch = async () => {
  ...
  try {
    const response = await axios.post('http://192.168.0.117:7008/HomePage/SetLineSetting', { isOpen: lineSwitch.value.checked });
    const data = response.data;
    if (data.state === 'success') {
