...
export default {
  ...
  setup() {
    ...
    async function getDetails() {
      try {
        const response = await axios.get(`http://192.168.0.117:7008/GetDBdata/GetIntegrationBoxInfo?id=${Integration.IntegrationId}`);
    ...
    async function submit() {
      ...
      const response = await axios.post('http://192.168.0.117:7008/IntegrationMng/ChangeEquipment', requestData);
      const data = response.data;
      try {
        console.log(data);
        if (data.state === 'success') {
