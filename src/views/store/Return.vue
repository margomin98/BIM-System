  ...
  setup() {
    ...
    async function submit() {
      try {
        ...
        const response = await axios.post('https://localhost:44302/AssetsInMng/OldAssetsIn', requestData);
    ...
    watch(() => formParams.AssetsId, async (newValue, oldValue) => {
      try {
        const response = await axios.get(`https://localhost:44302/GetDBdata/GetAssetInfo?id=${newValue}`);
        const data = response.data;
        if (data.state === 'success') {
