...
const saveUserRole = async () => {
  ...
  try {
    const reseponse = await axios.post('http://192.168.0.117:7008/AuthorityMng/AccoutChangeRole', form);
...
const getPagePermission = async (Id) => {
  ...
  form.append('Id', Id);
  try {
    const response = await axios.post('http://192.168.0.117:7008/AuthorityMng/GetRoleSettingById', form);
    const data = response.data;
    if (data.state === 'success') {
...
const saveRolePermission = async () => {
  ...
  try {
    const reseponse = await axios.post('http://192.168.0.117:7008/AuthorityMng/SetRoleSettingById', requestData);
    const data = reseponse.data;
    if (data.state === 'success') {
      alert(data.messages);
    } else {
