...
<script setup>
  ...
  function sendTextForm() {
    return new Promise((resolve, reject) => {
      ...
      axios.post('http://192.168.0.117:7008/ReceivingMng/CreateReceipt', requestJson)
        .then(response => {
  ...
  const getPurchaseNum = (() => {
    formParams.PO_ID = '';
    axios.get(`http://192.168.0.117:7008/GetDBdata/SearchPurchaseOrderID?id=${formParams.PurchaseNum}`)
    .then((r)=>{
      const data = r.data;
      if(data.state === 'success') {
