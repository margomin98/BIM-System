...
export const usePurchaseStore = defineStore('Purchase', {
  ...
  actions: {
    ...
    async submit(type) {
      ...
      try {
        const response = await axios.post('http://192.168.0.117:7008'+url,this.Form);
    ...
    async getDetails(PP_ID, Conditions) {
			try {
				const response = await axios.get(`http://192.168.0.117:7008/GetDBdata/ProjectPurchasingGetData?pp_id=${PP_ID}`)
				const data = response.data ;
				if(data.state === 'success') {
          if(Conditions) {
    ...
    // 刪除採購單
		async deleteData(PP_ID) {
			...
			try {
				const response = await axios.post(`http://192.168.0.117:7008/PurchasingMng/DeleteRequisition`, form);
				const data = response.data;
				if (data.state === 'success') {
